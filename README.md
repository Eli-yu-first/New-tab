...
*   **📑 Saved for Later**: Bookmark tabs to a checklist before closing them. Create persistent groups, then drag saved pages between them.
...
*   **⚡️ Tab Search**: Fuzzy search open tabs by title, full URL, or domain. Use `Cmd/Ctrl + K` to focus, arrow keys to choose, `Enter` to switch, and `Cmd/Ctrl + Backspace` to close.
...
*   **📑 Saved for Later (稍后阅读)**：保存待读网页清单，读完即可勾选划除；支持创建固定分组并通过拖拽整理网页。
...
*   **⚡️ 标签页模糊检索**：按标题、完整网址或域名定位打开的标签页。按 `Cmd/Ctrl + K` 聚焦，方向键选择，`Enter` 定位，`Cmd/Ctrl + Backspace` 关闭。
