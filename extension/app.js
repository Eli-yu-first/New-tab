...
const SAVED_TAB_GROUPS_STORAGE_KEY = 'tabOut_savedTabGroups';
...
async function processCloseRequests(requests) {
  for (const request of requests || []) {
    if (
      !request ||
      handledCloseRequestIds.has(request.id) ||
      request.browser !== getBrowserName() ||
      !request.url
    ) continue;

    handledCloseRequestIds.add(request.id);
    const allTabs = await chrome.tabs.query({});
    const matches = allTabs
      .filter(tab => isSimilarUrl(tab.url, request.url))
      .map(tab => tab.id);
    if (matches.length > 0) await chrome.tabs.remove(matches);
  }

  if (handledCloseRequestIds.size > 200) handledCloseRequestIds.clear();
}

function rememberDeletedHistoryUrl(url) {
  locallyDeletedHistoryUrls.add(url);
  while (locallyDeletedHistoryUrls.size > 1000) {
    locallyDeletedHistoryUrls.delete(locallyDeletedHistoryUrls.values().next().value);
  }
}

function getDeletedHistoryUrls() {
  const deletedUrls = new Set(locallyDeletedHistoryUrls);
  for (const deletion of sharedNativeData.historyDeletions || []) {
    if (deletion && deletion.url) deletedUrls.add(deletion.url);
  }
  return deletedUrls;
}

async function processHistoryDeletions(deletions) {
  for (const deletion of deletions || []) {
    if (!deletion || handledHistoryDeletionIds.has(deletion.id) || !deletion.url) continue;

    handledHistoryDeletionIds.add(deletion.id);
    rememberDeletedHistoryUrl(deletion.url);
    try {
      await chrome.history.deleteUrl({ url: deletion.url });
    } catch (err) {
      console.error('[tab-out] Failed to sync history deletion:', err);
    }
  }

  while (handledHistoryDeletionIds.size > 1000) {
    handledHistoryDeletionIds.delete(handledHistoryDeletionIds.values().next().value);
  }
}

async function deleteHistoryItem(url) {
  await chrome.history.deleteUrl({ url });
  rememberDeletedHistoryUrl(url);
  sharedNativeData.history = (sharedNativeData.history || []).filter(item => item.url !== url);
  if (nativePort) nativePort.postMessage({ action: 'deleteHistory', url });
}
...
function mergeSavedTabGroups(a, b) {
  const map = new Map();
  for (const group of a) {
    if (group && group.id && group.name) map.set(group.id, group);
  }
  for (const group of b) {
    if (group && group.id && group.name && !map.has(group.id)) map.set(group.id, group);
  }
  return Array.from(map.values());
}
  ...
  try {
    ...
    nativePort.onMessage.addListener(async (msg) => {
      ...
      if (msg && msg.event === 'changed') {
        ...
        await processCloseRequests(shared.closeRequests);
        await processHistoryDeletions(shared.historyDeletions);

        // 获取本地存储数据
        const syncResult = await chrome.storage.sync.get([
          'deferred',
          BOOKMARKS_STORAGE_KEY,
          SAVED_TAB_GROUPS_STORAGE_KEY,
        ]);
        const localDeferred = syncResult.deferred || [];
        const localBookmarks = syncResult[BOOKMARKS_STORAGE_KEY] || [];
        const localSavedTabGroups = syncResult[SAVED_TAB_GROUPS_STORAGE_KEY] || [];
        ...
        const mergedSavedTabGroups = mergeSavedTabGroups(shared.savedTabGroups || [], localSavedTabGroups);
        ...
        const localChanged = JSON.stringify(mergedDeferred) !== JSON.stringify(localDeferred) ||
                             ...
                             JSON.stringify(mergedSavedTabGroups) !== JSON.stringify(localSavedTabGroups);
        ...
        const sharedChanged = JSON.stringify(mergedDeferred) !== JSON.stringify(shared.deferred || []) ||
                              ...
                              JSON.stringify(mergedSavedTabGroups) !== JSON.stringify(shared.savedTabGroups || []);

        if (localChanged) {
          await chrome.storage.sync.set({
            deferred: mergedDeferred,
            [BOOKMARKS_STORAGE_KEY]: mergedBookmarks,
            [SAVED_TAB_GROUPS_STORAGE_KEY]: mergedSavedTabGroups,
          });
        }
        ...
        if (sharedChanged) {
          ...
          nativePort.postMessage({
            ...
            data: {
              ...
              deferred: mergedDeferred,
              savedTabGroups: mergedSavedTabGroups,
            }
          });
        }
  ...
  try {
    ...
    nativePort.postMessage({
      ...
      data: {
        ...
        savedTabGroups: syncResult[SAVED_TAB_GROUPS_STORAGE_KEY] || [],
      }
    });
  } catch (err) {
...
async function fetchOpenTabs() {
  try {
    ...
    const filteredLocal = tabs.filter(t =>
      t.url &&
      t.url !== newtabUrl &&
      isTrackableTabUrl(t.url)
    );
...
async function getSavedTabGroups() {
  const result = await chrome.storage.sync.get(SAVED_TAB_GROUPS_STORAGE_KEY);
  const groups = result[SAVED_TAB_GROUPS_STORAGE_KEY];
  if (!Array.isArray(groups)) return [];
  return groups.filter(group => group && typeof group.id === 'string' && typeof group.name === 'string');
}

async function persistSavedTabGrouping(deferred, groups) {
  await chrome.storage.sync.set({
    deferred,
    [SAVED_TAB_GROUPS_STORAGE_KEY]: groups,
  });
  await pushToNativeHost();
}

async function createSavedTabGroup(name) {
  const trimmedName = String(name || '').trim();
  if (!trimmedName) return null;

  const groups = await getSavedTabGroups();
  const group = {
    id: `saved-group-${Date.now().toString(36)}-${Math.random().toString(36).slice(2, 7)}`,
    name: trimmedName,
    createdAt: new Date().toISOString(),
  };
  await persistSavedTabGrouping(await getDeferred(), [...groups, group]);
  return group;
}

async function renameSavedTabGroup(id, name) {
  const trimmedName = String(name || '').trim();
  ...
  groups[index] = { ...groups[index], name: trimmedName };
  await persistSavedTabGrouping(await getDeferred(), groups);
  return true;
}

async function deleteSavedTabGroup(id) {
  if (!id) return false;
  const [deferred, groups] = await Promise.all([getDeferred(), getSavedTabGroups()]);
  ...
  const nextDeferred = deferred.map(tab =>
    tab.groupId === id ? { ...tab, groupId: '' } : tab
  );
  await persistSavedTabGrouping(nextDeferred, nextGroups);
  return true;
}

async function moveSavedTabToGroup(tabId, groupId) {
  const [deferred, groups] = await Promise.all([getDeferred(), getSavedTabGroups()]);
  const tabIndex = deferred.findIndex(tab => tab.id === tabId);
  if (tabIndex === -1) return false;
  if (groupId && !groups.some(group => group.id === groupId)) return false;

  deferred[tabIndex] = { ...deferred[tabIndex], groupId: groupId || '' };
  await persistSavedTabGrouping(deferred, groups);
  return true;
}

function buildSavedTabGroups(savedTabs, savedTabGroups) {
  const groups = (savedTabGroups || []).map(group => ({ ...group, tabs: [] }));
  const groupsById = new Map(groups.map(group => [group.id, group]));
  const ungrouped = { id: '', name: 'Ungrouped', tabs: [], isUngrouped: true };

  for (const tab of savedTabs || []) {
    const group = groupsById.get(tab.groupId);
    (group || ungrouped).tabs.push(tab);
  }
...
async function getHistoryItems() {
  try {
    ...
    for (const item of items) {
      if (!item.url || seen.has(item.url) || deletedUrls.has(item.url)) continue;
      seen.add(item.url);
      uniqueLocal.push({
        url: item.url,
    ...
    if (sharedNativeData && Array.isArray(sharedNativeData.history)) {
      for (const hist of sharedNativeData.history) {
        if (!hist || deletedUrls.has(hist.url)) continue;
...
async function checkTabOutDupes() {
  ...
  const tooltip = document.getElementById('tabOutDupeTooltip');
  if (!banner) return;
  ...
  try {
    ...
    const tabOutTabs = tabs.filter(t => {
      const url = t.url || '';
      return url.startsWith(newtabUrl) || url === 'chrome://newtab/' || url === 'chrome://new-tab-page/';
    });

    if (tabOutTabs.length > 1) {
      const extraCount = tabOutTabs.length - 1;
      const dict = LANG_DICT[currentLanguage] || LANG_DICT.zh;
      const message = dict.tabOutDupeTooltip.replace('{count}', String(extraCount));
      const button = banner.querySelector('.tab-cleanup-close');
      if (tooltip) tooltip.textContent = message;
      if (button) button.setAttribute('aria-label', message);
...
function buildOverflowChips(hiddenTabs, urlCounts = {}) {
  const hiddenChips = hiddenTabs.map(tab => {
    ...
    if (tab.remoteBrowser) {
      remoteBadge = ` <span class="remote-badge" style="font-size:9px; background:rgba(154,145,138,0.15); color:var(--muted); padding:1px 4px; border-radius:3px; margin-left:4px;">${escapeHtml(tab.remoteBrowser)}</span>`;
    }
 ...
 */
function renderSavedTabs(savedTabs, savedTabGroups) {
  ...
  function renderChip(tab) {
    ...
    return `<div class="page-chip clickable saved-chip" draggable="true" data-action="focus-tab" data-saved-tab-id="${safeId}" data-tab-url="${safeUrl}" title="${safeTitle}">
      ${faviconUrl ? `<img class="chip-favicon" src="${faviconUrl}" alt="" onerror="this.style.display='none'">` : ''}
      <span class="chip-text">${escapeHtml(label)}</span>
  ...
  return buildSavedTabGroups(savedTabs, savedTabGroups).map((group) => {
    const safeGroupId = escapeHtml(group.id);
    const groupActions = group.isUngrouped ? '' : `
      <div class="saved-group-actions">
        <button class="saved-group-action" data-action="rename-saved-group" data-group-id="${safeGroupId}" title="Rename group" aria-label="Rename group">
        ...
        <button class="saved-group-action saved-group-delete" data-action="delete-saved-group" data-group-id="${safeGroupId}" title="Delete group" aria-label="Delete group">
    ...
    const chips = group.tabs.map(renderChip).join('');

    return `
      <div class="mission-card simple-card saved-group-card" data-saved-group-id="${safeGroupId}">
        <div class="status-bar"></div>
        <div class="mission-content">
          <div class="mission-top">
            <span class="mission-name">${escapeHtml(group.name)}</span>
            <span class="open-tabs-badge">
              ${ICONS.tabs}
              ${group.tabs.length}
            </span>
          ...
          <div class="mission-pages saved-group-dropzone" data-saved-group-id="${safeGroupId}">${chips}</div>
        </div>
      </div>`;
  }).join('');
}
...
async function renderStaticDashboard() {
  ...
  const [savedTabsData, savedTabGroups] = await Promise.all([getSavedTabs(), getSavedTabGroups()]);
  if (savedTabsSection) {
    savedTabsSection.style.display = 'block';
    if (savedTabsData.active.length > 0 || savedTabGroups.length > 0) {
      ...
      savedTabsMissionsEl.innerHTML = renderSavedTabs(sortedSavedTabs, savedTabGroups);
    } else {
...
document.addEventListener('click', async (e) => {
  ...
  if (action === 'close-search-tab') {
    e.stopPropagation();
    await closeSearchTabById(actionEl.dataset.tabId);
    return;
  }

  if (action === 'rename-saved-group') {
    e.stopPropagation();
    openSavedGroupModal(actionEl.dataset.groupId);
    return;
  }
  ...
  if (action === 'delete-saved-group') {
    ...
    if (!groupId || !window.confirm('Delete this group? Saved tabs will stay ungrouped.')) return;
    if (await deleteSavedTabGroup(groupId)) {
  ...
  if (action === 'close-all-open-tabs') {
    ...
    const localTabs = openTabs.filter(t => Number.isInteger(t.id));
    if (nativePort) {
      const remoteCloseRequests = new Set();
      for (const remoteTab of openTabs.filter(t => t.url)) {
        const browsers = [remoteTab.remoteBrowser, ...(remoteTab.remoteBrowsers || [])].filter(Boolean);
        for (const browser of browsers) {
          const key = `${browser}\u0000${remoteTab.url}`;
          if (remoteCloseRequests.has(key)) continue;
          remoteCloseRequests.add(key);
          nativePort.postMessage({
            action: 'requestCloseTab',
            browser,
            url: remoteTab.url,
    ...
    const toClose = localTabs.map(t => t.id);
...
function setupSavedTabDragAndDrop() {
  const dropzones = document.querySelectorAll('.saved-group-dropzone');
  const chips = document.querySelectorAll('.saved-chip[data-saved-tab-id]');

  chips.forEach(chip => {
    chip.addEventListener('dragstart', event => {
      event.stopPropagation();
      event.dataTransfer.effectAllowed = 'move';
      event.dataTransfer.setData('text/plain', chip.dataset.savedTabId);
      chip.classList.add('dragging');
    });
  ...
  dropzones.forEach(dropzone => {
    ...
    dropzone.addEventListener('drop', async event => {
      ...
      const moved = await moveSavedTabToGroup(tabId, dropzone.dataset.savedGroupId || '');
      if (moved) await renderDashboard();
    });
  });
}
...
function openSavedGroupModal(groupId = null) {
  const modal = document.getElementById('savedGroupModal');
  const input = document.getElementById('savedGroupName');
  const title = document.getElementById('savedGroupModalTitle');
  if (!modal || !input) return;

  savedGroupEditingId = groupId;
  const group = groupId ? document.querySelector(`.saved-group-card[data-saved-group-id="${CSS.escape(groupId)}"] .mission-name`) : null;
  input.value = group?.textContent || '';
  if (title) title.textContent = groupId ? 'Rename group' : 'New group';
  modal.style.display = 'flex';
  input.focus();
  input.select();
}

function setupSavedGroupControls() {
  const addButton = document.getElementById('addSavedGroupBtn');
  const modal = document.getElementById('savedGroupModal');
  const input = document.getElementById('savedGroupName');
  const cancelButton = document.getElementById('savedGroupModalCancel');
  const saveButton = document.getElementById('savedGroupModalSave');
  if (!addButton || !modal || !input || !cancelButton || !saveButton) return;

  const closeModal = () => {
    modal.style.display = 'none';
    savedGroupEditingId = null;
  };

  addButton.addEventListener('click', () => openSavedGroupModal());
  cancelButton.addEventListener('click', closeModal);
  saveButton.addEventListener('click', async () => {
    const name = input.value.trim();
    const editingGroup = savedGroupEditingId;
    const saved = editingGroup
      ? await renameSavedTabGroup(editingGroup, name)
      : await createSavedTabGroup(name);
    if (!saved) {
      showToast('Enter a group name');
      return;
    }
    closeModal();
    showToast(editingGroup ? 'Group renamed' : 'Group created');
    await renderDashboard();
  });
  ...
  if (tabSearchInput) {
    ...
    tabSearchInput.addEventListener('keydown', async event => {
      ...
      if (event.key === 'ArrowDown' || event.key === 'ArrowUp') {
        ...
        const currentIndex = Array.from(results).findIndex(result => result.classList.contains('is-selected'));
        const direction = event.key === 'ArrowDown' ? 1 : -1;
        const nextIndex = currentIndex === -1
          ? (direction === 1 ? 0 : results.length - 1)
          : (currentIndex + direction + results.length) % results.length;
        selectTabSearchResult(nextIndex);
        return;
      }
      ...
      if (event.key === 'Escape') clearTabSearch();
    });
  }
...
function getTabSearchDomain(url) {
  try {
...
function findTabSearchMatches(query, tabs = openTabs) {
  ...
  return (tabs || []).map(tab => {
    ...
    const domain = getTabSearchDomain(url);
    let score = 0;

    for (const term of terms) {
      const titleScore = fuzzyScore(title, term);
      const domainScore = fuzzyScore(domain, term);
      const urlScore = fuzzyScore(url, term);
      if (!titleScore && !domainScore && !urlScore) return null;
...
function selectTabSearchResult(index) {
  const results = Array.from(document.querySelectorAll('.tab-search-result'));
  results.forEach((result, resultIndex) => {
    const selected = resultIndex === index;
...
async function closeSearchTabById(id) {
  const tab = getSearchTabById(id);
  if (!tab) return;
  if (Number.isInteger(tab.id)) {
    await chrome.tabs.remove(tab.id);
  } else if (nativePort && tab.remoteBrowser) {
    nativePort.postMessage({ action: 'requestCloseTab', browser: tab.remoteBrowser, url: tab.url });
  } else {
    return;
  }
  await fetchOpenTabs();
  renderTabSearchResults(document.getElementById('tabSearchInput')?.value || '');
  await renderDashboard();
}
  ...
  if (exportDataBtn) {
    exportDataBtn.addEventListener('click', async () => {
      try {
        const bookmarks = await getCustomBookmarks();
        const deferred = await getDeferred();
        const savedTabGroups = await getSavedTabGroups();
        const data = {
          bookmarks,
          deferred,
          savedTabGroups,
    ...
    importDataInput.addEventListener('change', (e) => {
      ...
      reader.onload = async (event) => {
        try {
          ...
          if (data.savedTabGroups && Array.isArray(data.savedTabGroups)) {
            const existingGroups = await getSavedTabGroups();
            const mergedGroups = mergeSavedTabGroups(existingGroups, data.savedTabGroups);
            await persistSavedTabGrouping(await getDeferred(), mergedGroups);
          }
...
const LANG_DICT = {
  zh: {
    ...
    // 多开新标签页清理提示
    tabOutDupeTooltip: "关闭其他 {count} 个新标签页"
  },
  en: {
    ...
    // Duplicate New Tab cleanup tooltip
    tabOutDupeTooltip: "Close {count} other New Tab pages"
  },
  ja: {
    ...
    // 重複した新しいタブの整理ヒント
    tabOutDupeTooltip: "他の新しいタブを {count} 個閉じる"
  }
};
